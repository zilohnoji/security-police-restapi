...
namespace SecurityPoliceMG.Authentication.Service.Impl;
    ...
    ITokenGenerator tokenGenerator) : IUserAuthService
{
    ...
    public CreateUserResponseDto ActiveAccount(string userEmail, string emailCode)
    {
        var userEntity = repositoryImpl.FindByEmailOrThrowNotFound(userEmail);

        if (!(userEntity.EmailCodeConfirmation != null && userEntity.EmailCodeConfirmation.Code.Equals(emailCode)))
        {
